...
export async function GET() {
  try {
    // Check which schema tables are in (including demo tables)
    const schemaInfo = await prisma.$queryRaw<Array<{table_schema: string, table_name: string}>>`
      SELECT table_schema, table_name
      FROM information_schema.tables
      WHERE table_name LIKE '%grants%' OR table_name LIKE '%vendors%' OR table_name LIKE '%profiles%'
...
export async function POST() {
  try {
    // Create demo tables with OpenAI embedding dimensions (1536)
    ...
    // Create indexes
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_demo_grants_status ON demo_discovered_grants(status)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_demo_grants_close_date ON demo_discovered_grants(close_date)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_demo_grants_agency ON demo_discovered_grants(agency)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_demo_vendors_sector ON demo_port_vendors(sector)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_demo_vendors_dbe ON demo_port_vendors(disadvantaged_business)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_demo_pipeline_profile ON demo_pipeline_grants(port_profile_id)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_demo_pipeline_stage ON demo_pipeline_grants(stage)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_demo_matches_grant ON demo_grant_vendor_matches(grant_id)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_demo_matches_vendor ON demo_grant_vendor_matches(vendor_id)`;

    return NextResponse.json({ success: true, message: "Demo tables created successfully" });
  } catch (err) {
    return NextResponse.json({
      error: err instanceof Error ? err.message : String(err),
    }, { status: 500 });
