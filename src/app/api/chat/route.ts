...
import { CORVUS_SYSTEM_PROMPT } from "@/lib/system-prompt";
import { procurementTools } from "@/tools/definitions";
...
export async function POST(req: Request) {
  if (!process.env.ANTHROPIC_API_KEY) {
    return new Response(
      JSON.stringify({ error: "ANTHROPIC_API_KEY is not configured. Add it to .env.local to use Corvo." }),
  ...
  try {
    ...
    // Check the latest user message for injection attempts and length
    ...
    if (containsInjection(userText)) {
      return new Response(
        JSON.stringify({ error: "I'm Corvus, the Corvo procurement analytics agent. I can help you analyze spend data, benchmark pricing, and identify savings opportunities. What would you like to explore?" }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
