...
import { ArrowLeft, BarChart3, AlertTriangle, CheckCircle2, Loader2 } from "lucide-react";
...
export default function SEFAView({ onBack }: SEFAViewProps) {
  ...
  const [sefa, setSefa] = useState<SEFAData | null>(null);
  const [loading, setLoading] = useState(true);

  const fetchSEFA = useCallback(async () => {
    if (tenant.isLoading) return;
    setLoading(true);
    try {
      const res = await fetch(`/api/awards/sefa?fyEnd=${fyEnd}`, {
        headers: { ...headers, "Content-Type": "application/json" },
      });
      if (res.ok) setSefa(await res.json());
    } catch { /* */ }
    setLoading(false);
  }, [fyEnd, headers, tenant.isLoading]);

  useEffect(() => { fetchSEFA(); }, [fyEnd, tenant.isLoading, tenant.portId]);
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Fiscal Year Ending {fmtDate(fyEnd)}</p>
          </div>
          <div className="shrink-0">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider block mb-1">Fiscal Year End</label>
            <select value={fyEnd} onChange={(e) => setFyEnd(e.target.value)} className="rounded-md border bg-background px-3 py-2 text-sm">
      ...
      {loading ? (
          ...
          <Card className={sefa.meetsAuditThreshold ? "border-amber-200 dark:border-amber-800" : ""}>
            <CardContent className="pt-4 pb-4">
              <div className="flex items-center gap-3">
                {sefa.meetsAuditThreshold ? <AlertTriangle className="h-5 w-5 text-amber-500" /> : <CheckCircle2 className="h-5 w-5 text-emerald-500" />}
                <div>
                  <p className="font-medium text-sm">Total Federal Expenditures: <span className="tabular-nums">{fmtFull(sefa.totalExpenditures)}</span></p>
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <Button variant="outline" size="sm" className="mt-3" onClick={fetchSEFA}>Retry</Button>
          </CardContent>
        </Card>
      )}
    </div>
