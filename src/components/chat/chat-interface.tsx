...
import { Send, Loader2, RotateCcw, Feather, AlertCircle } from "lucide-react";
...
export function ChatInterface() {
  ...
  // Throttled scroll: use instant scroll during streaming (no queued animations),
  // rate-limited to ~60fps via rAF to avoid layout thrashing
  const scrollToBottom = useCallback(() => {
    const now = Date.now();
    // During streaming, throttle to every 100ms to avoid jank
    ...
    <div className="flex flex-1 flex-col h-[calc(100vh-48px)]">
      ...
      <div className="flex items-center justify-between border-b px-6 py-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-sm font-semibold">Corvus Agent</h1>
            <p className="text-xs text-muted-foreground">20 procurement tools available</p>
          </div>
        </div>
        {messages.length > 0 && (
        ...
        <div className="mx-auto max-w-3xl px-4 py-6">
          {isEmpty ? (
            <div className="flex flex-col items-center justify-center pt-12">
              <div className="flex h-14 w-14 items-center justify-center rounded-lg bg-primary text-primary-foreground shadow-sm mb-4">
              ...
              <p className="mb-8 max-w-md text-center text-sm text-muted-foreground">
                Analyze your spend data, identify savings, benchmark pricing,
      ...
      <div className="border-t bg-card p-4">
        ...
        <p className="mx-auto mt-2 max-w-3xl text-center text-xs text-muted-foreground">
          Corvus uses 20 specialized procurement tools to analyze your data.
        </p>
      </div>
    </div>
  );
